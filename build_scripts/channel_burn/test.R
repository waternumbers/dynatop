...
devtools::load_all() ##library("dynatop")
ctch <- dynatopGIS$new(file.path(demo_dir,"example.tif"))
ctch$create_model("test",NULL)

## now try second stage - burning in channel and filling to outlet
list.files()
stage1_dem <- ctch$get_layer("filled_dem")
stage1_chn <- ctch$get_layer("channel")
stage1_vchn <- ctch$get_layer("channel_vect")

outlet_vchn <- stage1_vchn[stage1_vchn$band==0]
end_cell <- extract(stage1_dem,outlet_vchn,touches=TRUE,cells=TRUE)$cell[1]
end_chn <- buffer(vect(xyFromCell(stage1_dem, end_cell),crs=crs(stage1_dem)),1)
end_chn$startNode <- "a"
end_chn$endNode <- "b"
end_chn$length <- 1
end_chn$area <- 1
end_chn$slope <- outlet_vchn$slope
end_chn$name <- "end_chn"
cut_dem <- stage1_dem - 1*is.finite(stage1_chn)

stage2 <- dynatopGIS$new(file.path(demo_dir,"stage2.tif"))
stage2$add_catchment(catchment_outline)
stage2$add_dem(cut_dem)
stage2$add_channel(end_chn)
stage2$sink_fill()

cut_fd_dem <- stage2$get_layer("filled_dem")

plot(cut_fd_dem - cut_dem)
           ctch <- dynatopGIS$new(file.path(demo_dir,"example.tif"))
tmp <- as.polygons(stage1_dem)
...
outlet_vchn <- stage1_vchn[stage1_vchn$id==0]
