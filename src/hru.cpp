  ...
  sz_lnk_id(sz_lnk_id_), sz_lnk_frc(sz_lnk_frc_)
{
  ...
  case 2:
    ...
    break;
  case 3:
    // constant celerity and raf
    sf = std::make_unique<sfc_cnstC_raf>( sf_param_, Dx);
    break;
  // case 2:
...
void hru::init(std::vector<double> &vec_q_sf_in, std::vector<double> &vec_q_sz_in,
  ...
  // solve surface
  sf->init(s_sf,q_sf,q_sf_in,r_sf_rz);
	       ...
	       double const &vtol, double const &etol, int const &max_it, double const &Dt)
{
  ...
  // limites by 
  r_sf_rz =  sf->max_vert(s_sf,q_sf_in,Dt);
  ...
  // solve for surface
  sf->solve(s_sf, q_sf, q_sf_in, r_sf_rz, Dt);
