  ...
  s_sf(states_[0]), s_rz(states_[1]), s_uz(states_[2]), s_sz(states_[3])
{
  ...
  area = properties_[1] * properties_[2]; // width * Dx - parea propertiy of the hru
  map_area = properties_[0]; // area used to scale the inputs
  s_sf *= area;
  s_rz *= area;
  s_uz *= area;
  s_sz *= area;
...
void hru::init(std::vector<double> &vec_q_sf_in, std::vector<double> &vec_q_sz_in,
  ...
  double r_inj = map_area * r_uz_sz_0; // use map area since input is standardised by map area
  ...
  if( std::abs( sz->fq(s_sz) - q_sz ) > 1e-10 ){
    ...
    Rcpp::Rcout << s_sz << " " << sz->fq(s_sz) << std::endl;
  }
  ...
  if( std::abs( sf->fq(s_sf) - q_sf ) > 1e-10 ){
    ...
    Rcpp::Rcout << s_sf << " " << sf->fq(s_sf) << std::endl;
  }
	       ...
	       double const &vtol, double const &etol, int const &max_it, double const &Dt)
{
  ...
  Hzu = ub - s_sz + v_uz_sz + Dt*(q_sz_in - sz->fq(ub));
  ...
  if( Hzu < 0.0 ){
    ...
    while( (Hzu > vtol) and (it < max_it) ){ //((ub -lb) > vtol) and (it < max_it) ){
      ...
      Hz = z - s_sz + v_uz_sz + Dt*(q_sz_in - sz->fq(z));
  ...
  // surface
  s_sf += Dt*q_sf_in - v_sf_rz;
  sf->update(s_sf, q_sf, Dt, vtol, max_it);
