...
// [[Rcpp::export]]
void dt_exp_courant(Rcpp::DataFrame hillslope, // hillslope data frame
...
// [[Rcpp::export]]
void dt_exp_init(Rcpp::DataFrame hillslope, // hillslope data frame
...
// [[Rcpp::export]]
void dt_exp_implicit(Rcpp::DataFrame hillslope, // hillslope data frame
		     Rcpp::DataFrame channel, // channel data frame
		     Rcpp::DataFrame flow_direction, // flow directions data frame
  ...
  for(int it = 0; it < obs.nrow(); ++it) {
	...
	// mass balance contribution
      	...
      	mbv[3] -= chn_in; // volume lost from hillsope to channel
    ...
    for(int ii=0; ii < nchannel; ++ii){
      ...
      // prior to seperating channel inflow into two components channel_inflow(it,ii) = ch_in[ii]/timestep; //channel_inflow(it,ii)/timestep;
    }
