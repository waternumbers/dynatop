...
sfc_cnst::sfc_cnst(std::vector<double> const &param, double const &width, double const &area){
  kappa_1 = param[0]*width/area; // lower section
  kappa_2 = param[1]*width/area; // upper section
  s_1 = param[3];
}
double sfc_cnt::fk(double const &s){
  return( (kappa_1*std::min(s_1,s) + kappa_2*std::max(0,s-s_1))/s )
}
...
sfc_kin::sfc_kin(std::vector<double> const &param, double const &width, double const &grad, double const &area){
  kappa_1 = param[0]*width/area; // lower section
  kappa_2 = std::pow(width,2.0/3.0) * std::pow(grad,1.0/2.0) / param[1]; // param[1] is mannings n
  kappa_2 *= width*area; // so get cross sectional area
}
