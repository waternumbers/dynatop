...
\[
- \frac{ds_{sf}}{dt} = \sum q_{sf} + r_{sf \rightarrow rz}
\]
...
The unsaturated and saturated zones act in a coupled fashin subject
to the constraint 
\begin{equation} 0 \leq s_{uz} \leq s_{sz} \end{equation}

In many TOPMODEL and dynamic TOPMODEL forumations the governing ODE is written as

\begin{equation}
\frac{ds_{uz}}{dt} = r_{rz \rightarrow uz} - \frac{A s_{uz}}{T_d s_{sz}}
\end{equation}

however it is acknowledged that this formaulation has no physical
basis (Beven, The Primer, Box?? 20**)

Here we develop an equally parsimonious alternative informed by the
previsou analysis of (Bevn 1984, Ion, 1989 paper).
Let $K_0$ be the vertical saturated conductivity at the surface. On
the basis that saturated conductivity decreases with depth let, for
the upper most soil layer for some storage defict $z$
\[
K\left(z\right) = K_0 \exp\left(-\frac{z}{m}\right)
\]

Letting the lateral saturated conductivity for the soil later at
deficit $z$ be $\lambda K\left(z\right)$ the downslope flow is given
by

\[
\lambda K_0 \exp\left(-\frac{z}{m}\right) \tan\left(\beta\right)
\]

Integrating this over the saturated zone gives
\[
T\left(s_{sz}\right) =  \lambda K_0 \tan\left(\beta\right)
\int\limits_{s_{sz}}^{\infty} \exp\left(-\frac{z}{m}\right) dz \\
= \lambda K_0 \tan\left(\beta\right) 
\left[-m \exp\left(-\frac{z}{m}\right) \right]_{s_{sz}}^{\infty} \\
= \lambda m K_0 \tan\left(\beta\right) \exp\left(-\frac{s_{sz}}{m}\right)
\]

For vertical flow it is assumed that the flow interfaces saturate from
above so the inflow to the unsaturated zone is
$\min\left(r_{rz \rightarrow uz},K_{0}\right)$. However if the
...
\[
r_{uz \rightarrow sz} = K_0 \exp\left(-\frac{s_{sz}}{m}\right) \frac{s_{uz}}{s_{sz}}
\]


Flow from the
unsaturated zone to the saturated zone is
