...
where
\[
\eta = \frac{1}{2}\left( 1 - \frac{D v_{sf}}{c_{sf}^{2} \Delta x} \right)
\]
...
\left.q_{sf}^{-}\right\rvert_{t+\Delta t} - \left.s_{sf}\right\rvert_{t+\Delta t}
\]
Noting that bot the representative flow $q_{sf}$ and $\eta$ are functions of
$s_{sf}$ the value of $\left.s_{sf}\right\rvert_{t+\Delta t}$ can be found by
solving
\[
\left.q_{sf}\right\rvert_{t+\Delta t} = \left.\eta\right\rvert_{t+\Delta t} \left.q_{sf}^{-}\right\rvert_{t+\Delta t}
\]
...
1. Take an initial outflow estimate $\tilde{q}^* = \max \left(0,
   2\mathcal{G}\left(0,\Theta_{sz}\right) -
   2\mathcal{G}\left(\frac{\left.s_{sz}\right\rvert_{t}}{\Delta
   x},\Theta_{sz}\right) - \left.\tilde{q}_{sz}^{-}\right\rvert_{t+\Delta t} \right)$

2. For a number of iterations:

   a. Solve for $s_{sz}^{*}$ such that $\mathcal{G}\left(0,\Theta_{sz}\right) -
   \mathcal{G}\left(\frac{s_{sz}^{*}}{\Delta x},\Theta_{sz}\right) =
   \frac{1}{2}\left(\left.q_{sz}^{-}\right\rvert_{t+\Delta t} + q^{*} \right)$
   
   b. Compute $v_{uz \rightarrow sz}^{*} = A\Delta t \min\left(
\frac{\left( \left. s_{uz} \right.\rvert_{t} + \hat{v}_{rz \rightarrow uz}\right)}
{T_{d}s_{sz}^{*} + A \Delta t}, \frac{1}{T_{d}} \right)$

   c. Compute $\tilde{v}_{sz}^{*}$ from $s_{sz}^{*}$
   
   d. Take
   \[\tilde{q}_{*} = \max\left(0,\left(
   \frac{2\tilde{v}_{sz}^{*}}{2\tilde{v}_{sz}^{*}\Delta t + \Delta x}\right)
   \left( \left.s_{sz}\right\rvert_{t} + \frac{2\tilde{v}_{sz}^{*}\Delta t -
   \Delta x}{2\tilde{v}_{sz}^{*}}\left.q_{sz}^{-}\right\rvert_{t+\Delta t} -
   v_{uz \rightarrow sz}^{*}\right)\right)
   \]
   
3. Set $\left.s_{sz}\right\rvert_{t+\Delta t} = s_{sz}^{*}$ and $\left.q_{sz}^{+}\right\rvert_{t+\Delta t} = \tilde{q}^{*}$
