...
| Vertical fluxes            | $r_{sf \rightarrow rz}$ | Flow from the surface excess store to the root zone                      | m/s |
|                            | $r_{rz \rightarrow uz}$ | Flow from the root zone to the unsaturated zone                          | m/s |
|                            | $r_{uz \rightarrow sz}$ | Flow from unsaturated to saturated zone                                  | m/s |
| Lateral fluxes             | $q_{sf}$                | Lateral flow in the hillslope surface zone                               | m/s |
|                            | $q_{sz}$                | Lateral inflow to the hillslope surface zone                             | m/s |
...
The storage in the surface zone satisfies is split into two components. The
first compoent, identified by the super script $\left[1\right]$ receives the
lateral inflows, but has a maxium storage volume at which it spills to the
second component (identified by the super script $\left[2\right]$)

generates lateral outflow and drainf$s_{sf} \geq 0$.
...
variable parameter muskingham model (see Appendix A) as
\[
\frac{ds_{sf}}{dt} = \frac{1}{1-\eta_{sf}} q^{-}_{sf} 
- r_{sf \rightarrow rz} 
- \frac{1}{\kappa_{sf}\left(1 - \eta_{sf}\right)} s_{sf}
\]

Several interpretations of the scheme suiitable for the surface zone
are given in Table \@ref(tab:hs-sf-type).
...
| DifConst | Diffuse routing with constant celerity and diffusivity           |                         |           |
  ...
  Table: (\#tab:hs-sf-type) Outline of notation for describing a cross
  section of the hillslope HRU
...
approximation (see Appendix A) which gives

\begin{equation}
\frac{d \left(D-s_{sz}\right)}{dt} = 2q^{-}_{sz} 
+ r_{uz \rightarrow sz} 
- \frac{2}{\kappa_{sz}} \left(D-s_{sz}\right)
\end{equation}

or treating $D$ as constant and subsituting in the expressions for
$r_{uz \rightarrow sz}$ and $\kappa_{sz}$

\[
\frac{d s_{sz}}{dt} = -2q^{-}_{sz} 
- \frac{s_{uz}}{T_{d}s_{sz}} 
+ \frac{2c_{sz}}{\Delta x} \left(D-s_{sz}\right)
\]
...
The Kinematic approximation then requires a relationship between $q_{sz}$ and
 ...
 defict $h_{sz}$ as $A_{sz} = w\left(D-h_{sz}\right)$.
 This relationship is the transmissivity
...
a unit width such that $q_{sz} = w g\left(h_{sz},\Theta_{sz}\right)
 \geq 0$. 
 The celerity is computed as
 \[
 \frac{dq_{sz}}{dA_{sz}} = \frac{dq_{sz}}{dh_{sz}}\left(\frac{dA_{sz}}{dh_{sz}}\right)^{-1} =
 -\frac{d}{dh_{sz}}g\left(h_{sz},\Theta_{sz}\right)
 \]
 
 
 
 Since which satisfies
\begin{equation}
-\frac{dq_{sz}}{dh_{sz}} = -w\frac{d}{dh_{sz}}g\left(h_{sz},\Theta_{sz}\right) \geq 0
\end{equation}
and
\begin{equation}
g\left(h_{sz},\Theta_{sz}\right) \rightarrow 0 \quad \mathrm{as} \quad
 h_{sz}\rightarrow \infty
\end{equation}
...
that $q_{sz}$ decreases with increasing $h_{sz}$. This combined with
 the second condition ensures that the model represents a state where no
 lateral inflow is generated.

In soltuion of the governing ODE $s_sz$ is used in place of $h_{sz}$
in evaluating the celerity used to commput $\kappa$. However the
transmissivity also places a limit on the laterla flow into and out of
the saturated zone. To handle this, without introducing additional
dispersion, the inflows and outflows are repartitioned between the
surface and saturated lateral fluxes so that $q_{sz}^{-}$ and
$q_{sz}^{+}$ are less then or equal to 
$w^{+}g\left(0,\Theta_{sz}\right)$
...
outflow $q^{+}$ and lateral inflow $r$ along with the two possibly state or
time varying parameters $\eta$ and $\kappa$ as
...
\[
\frac{ds}{dt} = \frac{1}{1-\eta} q^{-} + r -
\frac{1}{\kappa\left(1-\eta\right)} s 
\]

## Approximating Tank Models

A tank model with possibly varying time constant $T$ takes $q^{+} =
...
considering, the area is rectangular and the lateral inflow $r$ uniuformly
...
\[
o \approx q + \eta \Delta x \frac{dq}{dx} + \frac{1}{2}\eta^2\Delta x^2 \frac{d^2 q}{dx^2}
\]
and
\[
i \approx q - \left(1-\eta\right) \Delta x \frac{dq}{dx} + \frac{1}{2}\left(1-\eta\right)^2 \Delta x^2 \frac{d^2 q}{dx^2}
\]
...
i - o \approx
-\Delta x \frac{dq}{dx} +
\frac{1}{2}\left(1-2\eta\right)^2\Delta x^2 \frac{d^2 q}{dx^2}
\]
...
Since $q = \frac{s}{\kappa}$ the chain rule and rearranging gives

\[
\kappa \frac{dq}{dt} + \Delta x \frac{dq}{dx} -
\frac{1}{2}\left(1-2\eta\right)\Delta x^2 \frac{d^2 q}{dx^2}
\approx \Delta x l
\]
...
The value of $\eta$ is limited. Firstly $\eta = 1/2$ results from $D=0$; that
is the Kinematic wave equation. Taking  $\eta=0$ produces the maxium
dispersion of $D = \frac{c\Delta x}{2}$. This imposes a relationship between reach length,
dispersion and celerity that can represented. In the case of $\eta=0$ the
system acts as a linear tank with $T = \frac{\Delta x}{c}$.
