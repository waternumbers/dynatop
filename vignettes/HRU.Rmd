...
how they may be used to capture a range of conceptual models. Muskingham-Cunge routing can be derived in a number of ways. Here we
offer a direct approach that reclaims the mass consrvative formulaton
given in Todini <2007>.

The surface storage $s_{sf}$ in a reach or hillslope of length $\Delta
x$ can be expressed in terms of the representative flow $q_{sf}$, velocity
$v_{sf}$ and cross-sectional area $a_{sf}$
as
\[
s_{sf} = \frac{q_{sf} \Delta x}{v_{sf}} = a_{sf} \Delta x
\]
