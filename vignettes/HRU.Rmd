...
\[
q_{sf}^{+} = \sum\limits_{i \in \mathcal{N}} 
\left( 
a_{1,i}\min\left(s_{sf},s_{1}\right)^{b_{1,i}} 
+ 
a_{2,i}\max\left(0,\min\left(s_{sf}-s_{1},s_{2}-s_{1}\right)\right)^{b_{2,i}} + 
a_{3,i}\max\left(0,s_{sf}-s_{2}\right)^{b_{3,i}} 
\right)
\]
This three layer structure can be made to match a variety of dynamics
as outlines in <TODO ref>
...
\[
q_{sz}^{+} = w \tan\left(\beta\right) T\left(s_{sz}\right)
\]
...
\]
It is acknowledged that this formaulation has no physical
basis (Beven, The Primer, Box?? 20**)
