...
terms of the storage deficit $s_{sz}$ from the maximum storage
$s_{sz}^{max}$; evolving as

\[
\frac{d}{dt}\left(s_{sz}^{max} - s_{sz}\right) = q_{sz}^{-} + r_{uz \rightarrow sz} - q_{sz}^{+}
\]
...
when $h_{sz}=0$ and zero when $h_{sz}=D$. The velocity over the cross
section is computed as

\[
v_{sz} = \frac{\mathcal{G}\left(h_{sz},\Theta_{sz}\right)}{
w\left(D-h_{sz}\right) }
\]

Assuming zero deficit when $q_{sz}^{-} = q_{sz}^{+} = q_{sz}^{max}$
the maximum storage is given by $s_{sz}^{max}=\Delta x w D$.
