...
<Beven et al.> propose a Kinematic subsurface approximation. The
flow across a given cross section is a function of the
depth to the saturated layer $h_{sz}$ with the functional form
$\mathcal{G}\left(h_{sz}, \Theta_{sz}\right)$ taking its maximum value
when $h_{sz}=0$ and zero when $h_{sz}=D$.
...
formulation. In applying the Muskingham-Cunge approach the velocity
can be computed as
\[
...
3. Set $\left.q_{sz}^{+}\right\rvert_{t+\Delta t} =
   \mathcal{G}\left(0,\Theta_{sz}\right) - q^{*}$ and
   compute
