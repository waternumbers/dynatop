...
$a = f\left(q, \Theta_{sf}\right)$. Using this the storage can be approximated by
\[
s_{sf} = \Delta x \left( \eta_{sf} f\left(q^{-}_{sf}\right) +
\left(1-\eta_{sf}\right) f\left(q^{+}_{sf}\right) \right)
\]

We consider only that case where $f$ is defined such that $f\left(0\right) = 0$
and  $\frac{da}{dq} >0$. The role of $\eta_{sf}$ in considered futher in
Appendix A which is summarised in Table \@ref(tab:hs-sf-type).and the numerical solution.
