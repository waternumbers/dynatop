...
Muskingham type approximation to the volume. Consistent with the
wedge derivation the storage $s_{sf}$ in an HRU of length $\Delta
x$ can be expressed in terms of the inflow $q^{-}_{sf}$, outflow $q^{+}_{sf}$ and state varying parameters $\nu_{sf}$ and $\eta_{sf}$ as

\[
s_{sf} = \frac{\Delta x}{\nu_{sf}} \left(\eta_{sf} q^{-}_{sf} + \left(1-\eta_{sf}\right) q^{+}_{sf} \right)
\]

Let the representative cross sectional area $a_{sf}$ be given by $a_{sf}=\frac{s_{sf}}{\Delta x}$. If $q_{sf} = \eta_{sf}
q^{-}_{sf} + \left(1-\eta_{sf}\right) q^{+}_{sf}$ is a representative
flow then $\nu_{sf}$ is the representative velocity since $\nu_{sf}=\frac{q_{sf}}{a_{sf}}$.
Appendix A shows that in many hydrological settings that $\eta_{sf}$ and $q_{sf}$ are functions of
$a_{sf}$ and hence $s_{sf}$. In the following (see Appendix B for details) consider that
\[
q^{+}_{sf} = \mathcal{F}\left(s_{sf},q^{-}_{sf}\right)
\]
where $q_{sf} \geq 0$ for all $s_{sf} \geq 0$ and
\[
...
approximation (see Appendix A). This gives a mass balance equation considered
...
The Muskingham approximation to the storage for the kinematic wave form gives

\[
D - s_{sz} = \frac{\Delta x}{\nu_{sz}} \left( 2q_{sz} - q^{-}_{sz}\right)
\]

The Kinematic approximation then requires a relationship between $q_{sz}$ and
 the cross sectional storage defict, given  defict
 $a_{sz}=\frac{s_{sz}}{\Delta x}$. This relationship defined using teh
 transmissivity, a one-to-one, continuously differentiable function 
$\mathcal{T}: \left[0,D\right] \rightarrow \mathcal{R}^{+}$ which returns the lateral flow on
a unit width such that $q_{sz} = w \mathcal{T}\left(a_{sz}/w,\Theta_{sz}\right)
 \geq 0$. The function $g$ is considered to satisify
\begin{equation}
-\frac{dq_{sz}}{da_{sz}} = -w\frac{d}{da_{sz}}\mathcal{T}\left(a_{sz}/w,\Theta_{sz}\right) \geq 0
\end{equation}
and
\begin{equation}
\mathcal{T}\left(a_{sz}/w,\Theta_{sz}\right) \rightarrow 0 \quad \mathrm{as} \quad
...
fluxes $\hat{r}_{* \rightarrow *} is determined. This then moderated in the
...
If the inputs and values at time $t$ are known the mass error for a four point solution with
time weighting parameter $\omega_sf$ is given by

\[
\mathcal{E}\left(\left. s_{sf} \right\rvert_{t+\Delta t},\omega_{sf}\right) = 
\left. s_{sf} \right\rvert_{t+\Delta t} + 
\Delta t \left( \omega_{sf} 
\left. q^{+}_{sf} \right\rvert_{t+\Delta t} + 
\left(1-\omega_{sf}\right)\left. q^{+}_{sf} \right\rvert_{t} 
\right) -
...
where the ouflow volume $\left. v_{sf}^{+} \right\rvert_{t+\Delta t}$ is given by
\[
\left. v_{sf}^{*} \right\rvert_{t+\Delta t} = 
\Delta t \left( \omega_{sf} 
\left. q^{+}_{sf} \right\rvert_{t+\Delta t} + 
\left(1-\omega_{sf}\right)\left. q^{+}_{sf} \right\rvert_{t} \right)
\]

A solution for the time $t+\Delta t$ satisfies
$\mathcal{E}\left(\left. s_{sf} \right\rvert_{t+\Delta t},\omega_{sf}\right) = 0$.

The four-point schem is numerically stable for $0.5 \leq \omega_{sf} \leq
1$. Since $\left. q^{+}_{sf} \right\rvert_{t+\Delta t}=0$ when $\left. s_{sf}
\right\rvert_{t+\Delta t} =0$ the maximum downwards flux occurs when
$\omega_{sf}=1$ and is given by
\[
\hat{v}_{sf \rightarrow rz} = \left. s_{sf} \right\rvert_{t} +
\left. v_{sf}^{-} \right\rvert_{t+\Delta t}
\]
...
By mass balance this gives
\[
v_{uz \rightarrow sz} = \frac{A \Delta t}
{T_{d}\left. s_{sz} \right.\rvert_{t+\Delta t} + A \Delta t}
\min\left( \left. s_{uz} \right.\rvert_{t} + \hat{v}_{rz \rightarrow uz},
\left. s_{sz} \right.\rvert_{t+\Delta t} + \frac{A \Delta t}{T_d} \right)
\]
...
The maximum downward flux is that which minimises $\left. s_sf
\right.\rvert_{t + \Delta t}$. Since for any positive lateral inflow there
...
Let the representave area $a$ be given by $a=\frac{s}{\Delta x}$. If $q = \eta
q^{-} + \left(1-\eta\right) q^{+}$ is a representative
...
\]
a solution can be found so long as $\mathcal{E}\left(s^{min},\omega\right)
\leq 0$

The solution is numerically stable so long as $0.5 \leq \omega \leq
1$ with second order accuray optained when $\omega = 0.5. Since increasing
$\omega$ adds numerical dispersion the solution uses the following cases:

1. If $\mathcal{E}\left(s^{min},0.5\right) \leq 0$ find $s_{t+\Delta t}$ by
   numerical search
2. If $\mathcal{E}\left(s^{min},0.5\right) > 0$ and
   ...
   \]
3. If $\mathcal{E}\left(s^{min},1\right) > 0$ take $\omega = 1$, $q_{t+\Delta
   t}=0$ and $s_{t+\Delta t} = s_{t} + v_{t+\Delta t}^{-} + v_{t+\Delta t}^{r}$
...
Muskingham routing can be derived in a numer of ways. Consistent with the
wedge derivation the evolution of the storage $s$ in a reach of length $\Delta
x$ can be expressed in terms of the cross sectional area of the inflow $a^{-}$
...
- select trial $\hat{s}$
- compute $\hat{a} = \max\left(0, \frac{1}{\Delta x \left(1-\eta\right)} \left( \hat{s} -
  \Delta x \eta a^{-}_{t+\Delta t}\right)\right)$
- evaluate $e = \hat{s} - s_{t} - \Delta t \left(q\left(a^{-}_{t+\Delta t}\right) +
  r_{t+\Delta t} - q\left(\hat{a}\right)\right)$
...
- compute $hat{z} = \Delta x \left(\eta v^{-}_{t+\Delta t} + \left(1-\eta\right)
\hat{v} \right)$
- compute $\hat{r} = \min\left( \frac{1}{T_{d}}, \frac{U}{T_{d}\hat{z} +
\Delta t} \right)$
- evaluate $e = \hat{z} - z_{t} +\Delta t \left(q\left(a^{-}_{t+\Delta t}\right) +
  \hat{r} - q\left(\hat{v}\right)\right)$

and search for $e=0$.

The derivative $\frac{de}{d\hat{z}} = 1 + \frac{d\hat{r}}{dz -
 }\frac{dq}{dv}\frac{dv}{d\hat{z}} > 0$. *TODO based on proof below*
 For a unique solution $e \leq 0$ for $hat{s}=0$. if this is not the case need
 to limit $\hat{r}$
  \[
 e = 0 - z_{t} +\Delta t \left(q\left(a^{-}_{t+\Delta t}\right) +
  \hat{r} \right)$
...
Further manipulations indicates that $\left.q^{+}\right.\rvert_{t + \Delta
t} \geq 0$ requires $\Delta t  \geq \kappa\eta$. *TODO - show these*
