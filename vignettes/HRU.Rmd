...
where
\[
\eta = \frac{1}{2}\left( 1 - \frac{2D v_{sf}}{c_{sf}^{2} \Delta x} \right)
\]
The dispersion $D_{sf}$ varies depending upon teh formulation required. A
diffusion wave approximation takes 
\[
D_{sf} = \frac{q_{sf}}{2BS_f} \approx  \frac{q_{sf}}{2BS_0}
\]
where for a Kinemativ wave formulation $D_{sf}=0$ and for a linear tank with
...
for with reference to the flow $q_{sf} = \frac{q_{sf}^{+}+q_{sf}^{-}}{2}$
...
flow across a give cross section is expressed as a function of the
unsaturated area $a_{sz}$ where the functional form
$\mathcal{G}\left(a_{sz}, \Theta_{sz}\right)$ takes its maximum value when $a_{sz}=0$.

Table \@ref(tab:hs-transmissivity) gives
$\mathcal{G}\left(a_{sz},\Theta_{sz}\right)$ for various the transmissivity
...
`dynatop` package <TODO rewrite in terms of a_sz>.
...
The kinematic assumption can be well captured by a Muskingum-Cunge
representation. Direct application of the method used for the surface zone is
not possible due to the inability to compute flow velocities for some
$\mathcal{G}\left(a_{sz},\Theta_{sz}\right)$ due to the lack of an upper bound
on $a_{sz}$. 

To adress this we instead formulate the model in terms of the difference
...
\]
which is directly ameanable to the approach taken for the surface. Since for a
kineatic wave $\eta=0.5$ this gives a representaive flow
\[
\tilde{q}_{sz} = \frac{1}{2}\left( \tilde{q}_{sz}^{-} + \tilde{q}_{sz}^{+} \right)
\]
...
\[
s^{*} = \frac{\Delta x}{v_{sf}} \eta_{sf} q_{sf}
\]
...
\frac{T_{d}\left(AD - \left. s_{sz} \right.\rvert_{t+\Delta t}\right)}{
...
1. Take an initial outflow estimate $\tilde{q}^* = \max \left(0,
   2\left( \mathcal{G}\left(0,\Theta_{sz}\right) -
   \mathcal{G}\left(\frac{\left.s_{sz}\right\rvert_{t}}{\Delta
   x},\Theta_{sz}\right)\right) - \left.\tilde{q}_{sz}^{-}\right\rvert_{t+\Delta t} \right)$

2. For a number of iterations:

   a. Solve for $s_{sz}^{*}$ such that
   $\mathcal{G}\left(\frac{s_{sz}^{*}}{\Delta x},\Theta_{sz}\right) = \mathcal{G}\left(0,\Theta_{sz}\right) -
   \frac{1}{2}\left(\left.\tilde{q}_{sz}^{-}\right\rvert_{t+\Delta t} + \tilde{q}^{*} \right)$
   
   b. Compute $v_{uz \rightarrow sz}^{*} = A\Delta t \min\left(
\frac{\left( \left. s_{uz} \right.\rvert_{t} + \hat{v}_{rz \rightarrow uz}\right)}
{T_{d}s_{sz}^{*} + A \Delta t}, \frac{1}{T_{d}} \right)$

   c. Compute $\tilde{v}_{sz}^{*} = \frac{\tilde{q}_{sz}^{*} \Delta x}{s_{sz}^{*}}$
   
   d. Take
   \[\tilde{q}^{*} = \max\left(0,\left(
   \frac{2\tilde{v}_{sz}^{*}}{2\tilde{v}_{sz}^{*}\Delta t + \Delta x}\right)
   \left( \left.s_{sz}\right\rvert_{t} + \frac{2\tilde{v}_{sz}^{*}\Delta t -
   \Delta x}{2\tilde{v}_{sz}^{*}}\left.\tilde{q}_{sz}^{-}\right\rvert_{t+\Delta t} -
   v_{uz \rightarrow sz}^{*}\right)\right)
   \]
   
3. Set $\left.\tilde{q}_{sz}^{+}\right\rvert_{t+\Delta t} = \tilde{q}^{*}$ and
   compute
   $\left.s_{sz}\right\rvert_{t+\Delta t} = \max\left( 0,\left.s_{sz}\right\rvert_{t} +
   \Delta t \left(\left.\tilde{q}_{sz}^{-}\right\rvert_{t+\Delta t} -
   \left.\tilde{q}_{sz}^{+}\right\rvert_{t+\Delta t}\right) - v_{uz \rightarrow sz}^{*}\right)$

## Upward Pass

Given an results from teh downward pass the upward pass starts with mass
...
2. For a number of iterations:
   ...
   b. Solve for $v_{sf}$ and $\eta_{sf}$ based on $q_{sf}$.
   
   c. Take
   \[q^{*} = \max\left(0,
	   \frac{v_{sf}}{\Delta x\left(1-\eta_{sf}\right) + \Delta t v_{sf}}
	   \left( \left.s_{s}\right\rvert_{t} - v_{sf \rightarrow rz} +
   \frac{v_{f} \Delta t - \Delta x \eta_{sf}}{v_{sf}} \left. q_{sf}^{-}
   \right.\rvert_{t+\Delta t} \right) \right)
   \]
