...
1. The most computationally intensive part of evaluating the gradients
   ...
   $\mathbf{A}^{-1}\left(\mathbf{W}_{sz}-\mathbf{I}\right)\mathbf{A}\left.\mathbf{l}_{sz}$. However
   this only needs to be evaluated once to get both the flux and storage
   gradient terms 
   and $\mathbf{q}_sz}$. This suggests that a potentially more accurate numerical
...
2. An alternative approximate solution can be derived by ignoring the limits
   ...
   multivariate non-homogeneous ODE problem for $\mathbf{l}_{sz}$. Implicit
   estimates for $\mathbf{q}_{sz}$ could then be applied.
