...
$h$ gives $A = \frac{h}{\Delta x}$. Further noting that $h=h_0-z$ for some
...
-\frac{1}{w c\left(Qw^{-1}\right)}\frac{dQ}{dt}
\]
where $c\left(Qw^{-1}\right)=-\frac{d}{dz}g\left(z\right)$ is the kinematic velocity
expressed as a function of lateral flow per unit width.

Subsitution gives
\[
\frac{1}{c\left(Qw^{-1}\right)}\frac{dQ}{dt} + \frac{dQ}{dx} -r = 0
\]
...
\left(1-\theta\right)c\left(\left.Q\right\rvert_{t}w^{-1}\right)
\left(\frac{\left.Q^{+}\right\rvert_{t}-\left.Q^{-}\right\rvert_{t}}{\Delta x} -r\right)
+ \\
& \theta c\left(\left.Q\right\rvert_{t+1}w^{-1}\right)\left(\frac{\left.Q^{+}\right\rvert_{t+1}-\left.Q^{-}\right\rvert_{t+1}}{\Delta x} -r\right)
= 0
\end{split}
\]
...
scheme replacing $c\left(Q\Delta x\right)$ with a 'typical' value $\bar{c}$ 
\[
\begin{split}
...
\[
\bar{c} = c\left(\bar{l}_{sz} w^{-1}\right)
\]
where
\[
\bar{l}_{sz} = \frac{1}{4}\left(
...
Since the  of $\left. l_{sz}\right\rvert_{t+\Delta t}$ is not known the approximation
\[
\check{l}_{sz} = \frac{1}{4}\left(
\left. l_{sz}\right\rvert_{t} +
\max\left(\left. l_{sz}\right\rvert_{t},\left. Q^{-}\right\rvert_{t+\Delta t}\right) +
\left. Q^{-}\right\rvert_{t+\Delta t}+
\left. Q^{-}\right\rvert_{t}
\right)
\]
is used in the evaluation of $\bar{c}$.
