...
A futher complication is the effect of saturation at $z=0$ which limits the
lateral flow to $Q\leq wg\left(0\right)$ at all points and times by producing
...
\[
l^{\left[i\right]}_{szmax}=w^{\left[i\right]}g^{\left[i\right]}\left(0\right)=g^{\left[i\right]}\left(0\right) /
...
value $\hat{c}$. A relatively simple approach to
constructing $\hat{c}$ is to take
\[
\hat{c} = \frac{1}{2}\left(\left.\bar{c}\right\rvert_{t} +
\left.\bar{c}\right\rvert_{t+\Delta t} \right)
\]
...
# Appendix: Exponential Transmissivity Profile
...
\]
The angle $\beta$ represents the slope of the HSU.
