...
-	To illisutrate some calculations intermediate evaluations variables are
     ...
     $q_{rz}$. Multiple $\tilde{}$ may be used if there are multiple
...
The surface excess store also interacts with to the channel. Let
$\tilde{\tilde{\mathbf{s}}}_{ch}$ be the contribution to the channel storage
from surface excess and
\[ \mathbf{x} = \left[ \begin{array}{c} \mathbf{s}_{ex} \\
                       \tilde{\tilde{\mathbf{s}}}_{ch} \end{array} \right] \]
...
The solution of this is carried out directly but through two stages. The first
stage redistributes the surface excess storage $\mathbf{s}_{ex}$ between the
hillslope and channel HRUs on the presumption that there is no inflow
($\mathbf{q}_{rz}=\mathbf{0}$). 
Specifically the ODE initial condition problem
...
\left.\tilde{\tilde{s}}_{ch}\right\rvert_{t+\Delta t} \end{array}\right]
$$
an intermediate solution to the channel storage and an approximation to the amount of water added
to the channel HRUs by surface excess redistribution.
...
For the $i$th hillslope HRU teh change in staturated zone storage deficit is
given by 
...
## Multiple variable non-homogeneous ODE with constant input
