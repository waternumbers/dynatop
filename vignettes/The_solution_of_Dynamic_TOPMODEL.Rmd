...
-	Where intermediate evaluations of some variables are compute these are distinguish by use of $\tilde{}$. For example
  ...
  Other HSU properties                   $f^{\left[k,j\right]}_{sf}$       Fraction of $l_{sf}$ going from $j$th              -
                                                                           HSU to $k$th HSU
  ...
  Vertical fluxes within HSU               $q_{sf \rightarrow rz}$         Flow from the surface excess store to the     (m$^3$/s)/m$^2$
                                                                           root zone
                                           $q_{rz \rightarrow uz}$         Flow from the root zone to the unsaturated    (m$^3$/s)/m$^2$
                                                                           zone                        
                                           $q_{rz \rightarrow sf}$         Flow from the root zone to the surface        (m$^3$/s)/m$^2$ 
                                                                           excess storage                  
                                           $q_{uz \rightarrow sz}$         Flow from unsaturated to saturated zone       (m$^3$/s)/m$^2$                        
                                           $q_{sz \rightarrow sf}$         Flow from saturated zone to surface           (m$^3$/s)/m$^2$
                                                                           excess storage                    
  Other HSU properties                               a                     Area                                              m$^2$
                                                $q_{sfmax}$                Maximum flow rate from surface excess to      (m$^3$/s)/m$^2$
                                                                           root zone
...
respectivly and $\bar{r} a constant inflow per unit length.
...
Consider the $i$ th HSU. The impact of saturation at
$s^{\left[i\right]}_{sz}=0$ is to limits the lateral flow to $l^{\left[i\right]}_{szmax}$ all points and times producing return flow to the
...
Suppose that $l_{szmaz}\geq\left.l_{sz}^{\left[i\right]}\right\rvert_{t}\geq 0$. 
This provides an estimate of $\left.Q^{+}\right\rvert_{t}$.
...
Constant recharge for every cross section means that
$\int_{\Delta t} q_{uz\rightarrow sz} dt = r \Delta x \Delta t$.

An initial estimate of the lateral flux from the saturated zone at the end of
...
\]
Assuming $\bar{c}^{\left[i\right]}$ and hence $\lambda^{\left[i\right]}$ is not dependent upon
