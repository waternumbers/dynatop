...
print( model$hru[,c('id','type','td')])
```
As in the above (where the unsaturated zone time constant is specified for the channel
HRU) there is no danger in specifying parameters not needed by the HRU type
...
check_obs(obs, unique( unlist(model$hru[,c("precip_input","pet_input")]) ))
```
...
number fo sub time steps used in the model evaluation which is discussed
later.
...
plot(out[,c('qobs','sim_1','sim_2','sim_3')])
```
