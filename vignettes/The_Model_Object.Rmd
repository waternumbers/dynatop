...
| map            | list             | Location of files for for creating maps of the HRUs       |
	
Table: (\#tab:model-table) Description of top level model components

## Options

The options vector is a named character vector. Two names variables are used:
...
[hillslope](https://waternumbers.github.io/dynatop/articles/hillslope_hru.html)
and
[channel](https://waternumbers.github.io/dynatop/articles/channel_hru.html)
vignettes for further details.

## Hillslope HRU table

Each row of the hillslope HRU table describes a single hillslope HRU. In this example it looks like:
```{r hillslope_parts}
head(model$hillslope)
```

The individual columns are documented in Table
\@ref(tab:hillslope-table). In this example the parameter columns relate to the exponential
transmissivity profile. The parameters in the table will with the
transmissivity options selected. 
...
| class   | integer | :-:   | Classification of HRU e.g. by a combination of topographic index and rainfall input                                                      |
...
| Name   | Class   | Unit  | Description                                                                                                                                              |
| ---    | ---     | ---   | -----                                                                                                                                                    |
| id     | integer | :-:   | Unique identifying number of the hillslope HRU. These, combined with the id from the hillslope table should for consecutive integers increasing from 1. |
...
The flow_direction data frame described the connections between the HRUs. 
...
are to which this series should be applied. From this an average precipitation
...
HRU. Each row contains a HRU id, the series to use and the fraction of the HRU
are to which this series should be applied. From this an average PET value
for each HRU is derived.
