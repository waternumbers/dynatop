...
h\left(x,t-\delta_{x\rightarrow g}\right) dx = \sum\limits_{i=1}^{n}q_i\left(g,t\right)
\]

Consider a single channel HRU. It is assumed that:

1. The channel represents a single reach of river such the a spatial location
   $x \in \mathcal{X}_{i}$ can be replaced by a unique length $l$ from the
   head of the reach $x_{i}$. The maximum value of $l$, denoted
   $\tilde{L}_{i}$ is the lower of the length of the reach $L_{i}$ or the
