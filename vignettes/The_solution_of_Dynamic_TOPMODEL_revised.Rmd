...
HRU is refered to only to the extend that an outcome of the computation is the
...
"hill slope" HRU.
  ...
  Vertical fluxes within HRU               $q_{rz \rightarrow uz}$         Flow from the root zone to the unsaturated zone                        (m$^3$/s)/m$^2$
                                           ...
                                           $q_{uz \rightarrow sz}$                  Flow from unsaturated to saturated zone                                (m$^3$/s)/m$^2$
                                                 $q_{sz \rightarrow ex}$                  Flow from saturated zone to surface excess storage                    (m$^3$/s)/m$^2$
  ...
  Other HRU properties                               a                     Area                                                                        m$^2$
                                         $w^{\left[i,j\right]}_{ex}$       Fraction of $l_{ex}$ going from $j$th to $i$th "hill slope" HRU              -
                                         $w^{\left[i,j\right]}_{sz}$       Fraction of $l_{sz}$ going from $j$th to $i$th "hill slope" HRU              -
                                         $f^{\left[k,j\right]}_{ex}$       Fraction of $l_{ex}$ going from $j$th hill slope HRU to $k$th channel HRU    -
                                         $f^{\left[k,j\right]}_{sz}$       Fraction of $l_{sz}$ going from $j$th hill slope HRU to $k$th channel HRU    -                                        
...
the additional notation for properties of the channel HRU which are used.
  ...
  Other HRU properties              a_{ch}               Area   m$^2$
                                    v_{ch}             Total inflow from hillslope   m$^3$
...
It is presumed that, from the perspective ofthe hillslope the channel acts as
a sink. That is there is no feedback from the channel to the hillslope. 
For a single reach:
\[
\frac{dv^{\left[k\right]}_{ch}}{dt} = 
...
\[
\frac{d\mathbf{v}_{ch}}{dt} = 
    \mathbf{F}_{ex}\mathbf{A}\mathbf{l}_{ex}
    + \mathbf{F}_{sz}\mathbf{A}\mathbf{l}_{sz}
    + \mathbf{A}_{ch}p
\]

The solution presumes that $\left.\mathbf{v}_{ch}\right\rvert_{t}=\mathbf{0}$
with the integral of each of the summed terms being evaluated individually to
give the total $\left.\mathbf{v}_{ch}\right\rvert_{t+\Delta t}$, the total
flux to the channel over the timestep.
...
+ \mathbf{A}\mathbf{q}_{sz \rigtharrow ex}
+ \mathbf{A}\mathbf{q}_{rz \rightarrow ex}
- \mathbf{A}\mathbf{q}_{ex \rightarrow rz}
\]

The surface excess store also passes water to the channel. Let $\mathbf{v}_{ex
\rightarrow ch}$ be volume contributed to the channel HRUs from the surface
excess store and 
\[
\mathbf{x} = \left[ \begin{array}{c} \mathbf{s}_{ex} \\
    \mathbf{v}_{ex \rightarrow ch} \end{array}\right] 
\]
...
\[
     \frac{d\mathbf{x}}{dt} =
	 \left[ \begin{array}{cc} \mathbf{A} & \mathbf{0} \\
         \mathbf{0} & \mathbf{I} \end{array} \right]^{-1}
	      \left[ \begin{array}{cc}
		  \begin{array}{c} \left(\mathbf{W}_{ex}-\mathbf{I}\right)\mathbf{A}\mathbf{T}^{-1}_{ex} \\
			  ...
			  \right]
          \mathbf{x} +
			   \left[ \begin{array}{c} \mathbf{q}_{sz} + \mathbf{q}_{rz \rightarrow ex}- \mathbf{q}_{ex \rightarrow rz}\\
                       \mathbf{0} \end{array}\right]
\]
	
The solution of this is carried out through two stages. 
The first stage redistributes the surface excess storage
$\left.\mathbf{s}_{ex}\right\rvert_{t}$ 
between the hill slope and channel HRUs while ignoring the potential
vertical flux within the HRUs (
$\mathbf{q}_{sz} + \mathbf{q}_{rz \rightarrow ex} - \mathbf{q}_{ex \rightarrow
  rz}=\mathbf{0}$
). The second stage, carried out in later steps corrects for the vertical fluxes.

Specifically letting
\[ \tilde{\mathbf{x}} = \left[ \begin{array}{c} \tilde{\mathbf{s}}_{ex} \\
                       \tilde{\mathbf{v}}_{ex \rigtharrow ch} \end{array} \right] \]
...
\left[\begin{array}{c} \tilde{s}_{ex} \\
    \left.\tilde{\mathbf{v}}_{ex \rigtharrow ch}\right\rvert_{t+\Delta t} -
    \left.\tilde{\mathbf{v}}_{ex \rigtharrow ch}\right\rvert_{t}
  \end{array}\right]
\]
This represents an intermediate solution to the surface excess storage and
approximation to the flux from the surface excess store to the channel HRUs.
...
For a single HRU
\[
\frac{ds_{rz}}{dt}=p + q_{ex \rightarrow rz} -\frac{s_{rx}}{s_{rzmax}}e_{p}-q_{rz\rightarrow ex} - q_{rz\rightarrow uz}
\]
Assuming all the water in the surface excess store is avaialbe to the root
...
\[
\int\limits_{\Delta t} \tilde{q}_{ex \rightarrow rz} dt =
\min\left(q_{exmax}\Delta t,\tilde{s}\right)
\]
Using this the root zone is solved as a two step process. Firstly the ODE
\[
\frac{d\tilde{s}_{rz}}{dt}=p + \frac{1}{\Delta t}\int\limits_{\Delta t}
\tilde{q}_{ex \rightarrow rz} dt -\frac{e_{p}}{s_{rzmax}}\tilde{s}_{rz} \quad
...
t\right) + \left(p + \frac{1}{\Delta t}\int\limits_{\Delta t}\right)\frac{
  s_{srmax}}{e_p}\left(1 - \exp\left(-\frac{e_{p}}{s_{rzmax}}\Delta
t\right)\right)
\]
