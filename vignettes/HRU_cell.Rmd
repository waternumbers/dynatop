...
\[
- \frac{ds_{sf}}{dt} = r_{sf \rightarrow rz} + \sum\limits_{i=1}^{8}  q_{sf,i} 
\]
...
The outflow over the $i$th boundary is given with reference to the hydraulic
gradient $S_{h,i}$ and cross sectional flow area $a_{sf_i}$ using the Mannings
formula for velocity
\[
v_{sf,i} = \mathrm{sign}\left(s_{h,i}\right) \frac{1}{n}R_{h}\left(a_{sf,i}\right) \left|S_{h,i}\right|^{0.5}
\]

Differing formulations are derived by differing approximations to the
hydraulic gradient. The two considered are as follows

- Kinematic Approximation where the hydrualic graient equal the bed slope so
  $S_{h,i} = S_{0,i}$
- Poor mans diffuse approximation where for constant $A$ the aproximation is
  given by $S_{h,i} = \max\left(0, S_{0,i} + \frac{1}{A}\frac{d}{dx}
  s_{sf}\right)$, which does not allow upslope flow, but stops downslope flow
  when the surface water evelations match.
...
\begin{equation}
\frac{ds_{uz}}{dt} = r_{rz \rightarrow uz} - \frac{A s_{uz}}{T_d \left(AD - s_{sz}\right)}
\end{equation}

If water is able to pass freely to the saturated zone, then it flows at the rate
$\frac{A s_{uz}}{T_d \left(AD - s_{sz}\right)}$. 
In the situation where $AD - s_{sz}=s_{uz}$ the subsurface below
...
terms of the storage $s_{sz} <= AD$ as:
\[
\frac{ds_{sz}}{dt} = r_{uz \rightarrow sz} - \sum\limits_{1}^{8} q^{sz,i}
\]
where when $s_{sz} = AD$ the flow from the unsaturated zone becomes negative.

The Kinematic subsurface approximation allows the lateral fluxes to be
expressed in terns of the surface gradient, flow croess section area
$a_{sf_i}$ and a velocity dependent upon the storage
$\mathcal{v}\left(s_{sz}\right) as
\[
q_{sz,i} = a_{sf,i} S_{0,i} \mathcal{v}\left(s_{sz}\right)
\]
...
solution of the saturated zone, with an upward pass giving the final states.
  2. The kinematic (or poor mans diffuse) approximations allow an ordering of
     the HRUs which allows a solution to be derived based on the premis that
...
Given the ordering of the HRUs we are interested only in the evaluating the
velocities across to those HRUs downslope. Given this and the desire for the
evaluation o feach HRU to be independent take
\[ 
\tilde{a}_{sf,i} = \frac{w_{i}}{A}\left.s_{sf}\right|_{t}
\]
and subsequently
\[
\tilde{v}_{sf,i} = \frac{1}{n}R_{h}\left(\tilde{a}_{sf,i}\right) \left|S_{0,i}\right|^{0.5}
\]

With $\left. q_{sf}^{-} \right\rvert_{t+\Delta t}$ being the already
determined inflows and, by convention $w_i=0 \iff S_{0,i} \leq 0$ The implicit solution of mass balance equation gives

\[
\left. s_{sf} \right\rvert_{t+\Delta t} = 
\left( 1 + \Delta t \sum\limits_{i=1}^{8} \frac{w_{i}\tilde{v}_{sf,i}}{A} \right)^{-1}
\left(
...
T_{d}\left(AD - \left. s_{sz} \right.\rvert_{t+\Delta t}\right) + A \Delta t}
\left( \left. s_{uz} \right.\rvert_{t} + 
v_{rz \rightarrow uz}\right)
\end{equation}

Since $\left. s_{uz} \right.\rvert_{t+\Delta t} \leq AD - \left. s_{sz}
	   \right.\rvert_{t+\Delta t}$ this places an additional condition of
\begin{equation}
\left. s_{uz} \right.\rvert_{t} + v_{rz \rightarrow uz} \leq 
AD - \left. s_{sz} \right.\rvert_{t+\Delta t} + \frac{A \Delta t}{T_d}
\end{equation}
...
{T_{d}\left(AD - \left. s_{sz} \right.\rvert_{t+\Delta t}\right) + A \Delta t}, \frac{1}{T_{d}} \right)
\]
which is apporximated as
\[
\hat{v}_{uz \rightarrow sz} = A\Delta t \min\left(
\frac{\left( \left. s_{uz} \right.\rvert_{t} + \hat{v}_{rz \rightarrow uz}\right)}
{T_{d}\left(AD - \left. s_{sz} \right.\rvert_{t} \right) + A \Delta t}, \frac{1}{T_{d}} \right)
\]


### Saturated Zone

As with the surface zone the ordering of the HRUs means we are interested only in the evaluating the
velocities across to those HRUs downslope. Given this and the desire for the
evaluation of each HRU to be independent take
\[ 
a_{sz,i} = \frac{w_{i}}{A}\left.s_{sf}\right|_{t+\Delta t}
\]
and an explicit approximation of the velocity
\[
\tilde{v}_{sz,i} = \mathcal{v} \left( \left.s_{sz}\right\rvert_{t} \right)
\]
With $\left. q_{sz}^{-} \right\rvert_{t+\Delta t}$ being the already
determined inflows and, by convention $w_i=0 \iff S_{0,i} \leq 0$ The implicit solution of mass balance equation gives

\[
\left. s_{sz} \right\rvert_{t+\Delta t} = \min\left(AD,
\left( 1 + \Delta t \sum\limits_{i=1}^{8} \frac{w_{i}\tilde{v}_{sf,i}}{A} \right)^{-1}
\left(
\left. s_{sz} \right\rvert_{t} +
\Delta t 
\left. q_{sz}^{-} \right\rvert_{t+\Delta t} +
\left. v_{uz \rightarrow uz} \right\rvert_{t+\Delta t}
\right)
\right)
\]
