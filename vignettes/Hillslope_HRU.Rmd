...
\right\rvert_{\Delta t}$ is conditional upon whichh side of $s_{raf}$
it falls.
If 
\[
s_{raf} \lt 
...
A direct implimentation of the above results takes the form of Algorithm 1.

***
*Algorithm 1: Direct Implimentation*
...
The challange with Algorithm 1 is that maintaining a water (mass) balance and
...
such that for some numerical tolerence $\epsilon$ we have $\left\lvert
...
- If $h(z_{max}) < 0$ (poosible only due to the outflow flux approximation) then
...
The implimenation of this approach is presented in Algorithm 2. it can be seem
...
***
*Algorithm 2: Stable Implimentation*
...
Using the monoticity of $h\left(z\right)$ we solve each part
seperatly. Firstly if $h\left( z_{C} \right) \geq 0$ then the inflow from the unsaturated
  zone is $1/T_{d}$ and 
  \[
  \left. \bar{s}_{sz}\right.\rvert_{\Delta t} =
  \left. \bar{s}_{sz}\right.\rvert_{0} +
...
\[
0 = T_{d} z^2 - T_{d} z \left(\left. \bar{s}_{sz}\right.\rvert_{\Delta 0} +
	\frac{\Delta t}{\Delta x} \left(l_{sz}^{+} -
		l_{sz}^{-}\right) \right) +
	\Delta t z -
	\Delta t \left(\left. \bar{s}_{sz}\right.\rvert_{\Delta 0} +
		\frac{\Delta t}{\Delta x} \left(l_{sz}^{+} -
		l_{sz}^{-}\right) \right)
		 +
\Delta t \left(
...
\]
\[
0 = z^2 - z \left(\left. \bar{s}_{sz}\right.\rvert_{\Delta 0} +
	\frac{\Delta t}{\Delta x} \left(l_{sz}^{+} -
...
\[
-b = \left. \bar{s}_{sz}\right.\rvert_{\Delta 0} +
	\frac{\Delta t}{\Delta x} \left(l_{sz}^{+} -
		l_{sz}^{-}\right)  - \frac{\Delta t}{T_{d}}
\]

and *THIS IS WRONG _ SHOULD NOT USE H(ZC)*
\[
c = \frac{\Delta t}{T_{d}} h\left(z_{C}\right)
\]
Since $z_{C}>0$ and the solution is valid when $h\left(z_{C}\right) < 0$ we
see that $-b > 0$ and
