...
hillslope_volume <- function(mdl,ignore_sz=FALSE){
  tmp <- mdl$hsu$hillslope
  if(!ignore_sz){
    sum( tmp$attr$area * (tmp$state$sf + tmp$state$rz + tmp$state$uz - tmp$state$sz) )
  }else{
    sum( tmp$attr$area * (tmp$state$sf + tmp$state$rz + tmp$state$uz) )
  }
}
```
...
base_model$hsu <- initialise(base_model,0.015)
```
...
for(ii in c("rz","uz")){
  initial_mdl$hsu$hillslope$state[[ii]][] <- 0
}
initial_mdl$hsu$hillslope$state[['sz']][] <- 1e10
initial_mdl$hsu$hillslope$state[['sf']][] <- 0.1
```

Set the `qsf_max` so that no water is absorbed back into the soil and
`lsz_max` so that no water drains from the saturated store
```{r}
initial_mdl$hsu$hillslope$param[["qsf_max"]][] <- 0
initial_mdl$hsu$hillslope$state[["lsz_max"]][] <- 0
```
...
abs(initial_vol - (final_vol + channel_vol)) / sum(out$model$hsu$hillslope$attr$area)
```
...
```{r}
initial_mdl <- base_model
for(ii in c("sf","uz","sz")){
  initial_mdl$hsu$hillslope$state[[ii]][] <- 0
}
```

Set the `qsf_max` so that no water is absorbed back into the soil from the
surface store and
`lsz_max` so that no water drains from the saturated store
```{r}
initial_mdl$hsu$hillslope$param[["qsf_max"]][] <- 0
...
initial_mdl$hsu$hillslope$state[['uz']][] <- 0.1
initial_mdl$hsu$hillslope$state[['sz']] <- initial_mdl$hsu$hillslope$state[['uz']]+1
```
...
initial_mdl$hsu$hillslope$param[["qsf_max"]][] <- 0
```
imulate the model and compute volume balance per unit area.
```{r}
out <- dynatop(initial_mdl,obs,use_states=TRUE)
initial_vol <- hillslope_volume(initial_mdl)
final_vol <- hillslope_volume(out$model)
...
final_vol <- hillslope_volume(out$model)
channel_vol <- sum(out$channel_input) * unique(diff(.index(out$channel_input)))
precip_vol <- sum(obs$rain)*sum(c(initial_mdl$hsu$hillslope$attr$area,
                                  initial_mdl$hsu$channel$attr$area))
abs(initial_vol + precip_vol- (final_vol + channel_vol)) / sum(out$model$hsu$hillslope$attr$area)
