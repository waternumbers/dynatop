...
**WHILE CORRECT THIS USES A NOTATION DIFFERENT TO THE REMAINDER OF THE VIGNETTES**
...
# Inputs to a channel length

A channel length is considered as having two types of inflow, labelled diffuse
and point. Diffuse infloes are considered as external inputs which occur at a
constant rate of the length of the reach. These include precipitation, the
inflow from Hillslope HRUs and time series specified in the `diffuse_inputs`
table.

Point inflow are considered to occur at the head of the channel length. These
include inflow from reaches above that being considered and time series
specified in the `point_inputs` table.
...
water in the channel based on assigning a constant velocity to each channel
length. Consider that the inflow data have a regular time step $\Delta t$
over which the flow rate is constant and we wish to evaluate the outflow at
some point on the river network on the same regular time step. To do this a
time delay histogram is constructed for each channel length and series in the
`point_inflow` table.

Utilisng the channel connectivity, channel lengths and velocities the time
$t_{min}$ which it takes water from the foot of the channel length to reach the gauge can be
computed. Similarly the time taken to reach the gauge from the head of the
channel length is given by $t_{max} = t_{min} + L/v_{ch}$.

Let $i$ be the largest integer less then $t_{min}/\Delta t$ and $j$ the
largest integer less then $t_{max}/\Delta t$.

the evaluation of the averageflow a given gauge point on the
river at a regular time step $\Delta t$. solution ofTo compute the For a given gauge at which the flow is required on a Using the channel netwoekIt
is 
