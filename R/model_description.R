...
model_description <- function(type=c("hillslope","channel","point_inflow","gauge"),include_states=FALSE,include_tmp=FALSE){
    ...
    if(type=="hillslope"){
        out <- data.frame(name = c("id","atb_bar","s_bar","area","delta_x","sz_band","sf_band", # attributes associated with catchment HSU
                                   ...
                                   "p","e_p","l_sf","q_sf_rz","q_rz_sf","q_rz_uz","q_uz_sz","q_uz_sf","q_sz_sf","e_t","l_sf","vol_l_sz_in","Q_minus_t","Q_plus_t","Q_minus_tDt"), ## tempory stores not needed for next timestep
