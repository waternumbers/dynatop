#' Routing by matrix exponetial method
#'
#' @description Function to setup the exponential of the matrix for an exponential matrix routing
...
#' @export
expm_setup <- function(K,dt){
    eig = eigen(K*dt)
    eig$vectors%*%diag(exp(eig$values))%*%solve(eig$vectors)
}
